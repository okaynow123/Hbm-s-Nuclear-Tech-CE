 ...
 * buffer of the last second's velocity values and generates a predicted target based on the average
 * speed.
 *
 * @author hbm
 */
public class RocketTargetingPredictive implements IRocketTargetingBehavior {
  ...
  public void recalculateTargetPosition(EntityArtilleryRocket rocket, Entity target) {
    ...
    if (eta <= 1) {
      rocket.setTarget(
          target.posX, target.posY - target.getYOffset() + target.height * 0.5D, target.posZ);
      return;
    }

    /* generate averages and predict a new position */
    double predX = target.posX + (motionX / 20D) * eta;
    double predY = target.posY - target.getYOffset() + target.height * 0.5D + (motionY / 20D) * eta;
