...
public class SkyProviderCelestial extends IRenderHandler {
	...
	public void render(float partialTicks, WorldClient world, Minecraft mc) {
		...
		Vec3d pos = mc.player.getLook(partialTicks);
		...
		GL11.glPushMatrix();
		{
			...
			GL11.glColor4f(sunBrightness, sunBrightness, sunBrightness, ((float)pos.y - 200.0F) / 300.0F);
		...
		if(sunsetColor != null) {
			...
			GL11.glPushMatrix();
			{
				...
				for(int j = 0; j <= segments; ++j) {
					...
					float sinAngle = (float) MathHelper.sin(angle);
					float cosAngle = (float) MathHelper.cos(angle);
	...
	protected void renderSun(float partialTicks, WorldClient world, Minecraft mc, double sunSize, double coronaSize, float visibility, float pressure) {
		...
		if(SolarSystem.kerbol.shader != null && SolarSystem.kerbol.hasTrait(CBT_Destroyed.class)) {
			...
			GL11.glColor4f(1.0F, 1.0F, 1.0F, 1 - MathHelper.clamp(pressure, 0.0F, 1.0F) * 0.75F);
