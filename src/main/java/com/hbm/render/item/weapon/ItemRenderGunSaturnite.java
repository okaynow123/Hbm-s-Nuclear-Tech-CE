	...
	public void renderByItem(ItemStack item) {
		...
		case FIRST_PERSON_RIGHT_HAND:
			...
			if (type == TransformType.FIRST_PERSON_RIGHT_HAND) {
				...
				GlStateManager.translate(1.6, 1.4, 0.8);
				GL11.glRotated(180, 1, 0, 0);
			...
			if(GunBoltAction.getRotationFromAnim(item) > 0) {
				GlStateManager.rotate(GunBoltAction.getRotationFromAnim(item) * 10, 2.5F, 0.0F, 1.5F);
				GlStateManager.translate(GunBoltAction.getOffsetFromAnim(item) * -1.75F, 0.0F, 0.0F);
			}
		...
		case HEAD:
			GlStateManager.scale(0.5F, 0.5F, 0.5F);
			GlStateManager.translate(1.0, 0.9, 0.9);
