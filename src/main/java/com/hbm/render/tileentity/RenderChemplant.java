...
public class RenderChemplant extends TileEntitySpecialRenderer<TileEntityMachineChemplant> {
	...
	public void renderExtras(TileEntity tileEntity, double x, double y, double z, float f) {
		...
		if(chem.tanks[0].getTankType() != null && chem.isProgressing)
		...
		if(chem.tanks[1].getTankType() != null && chem.isProgressing)
			GL11.glRotatef(rotation, 0F, 1F, 0F);
		else
			GL11.glRotatef(45, 0F, 1F, 0F);
        ...
        if(chem.tanks[0].getTankType() != null) {
        	ResourceLocation test;
        	if(chem.tanks[0].getTankType() == Fluids.LAVA || chem.tanks[0].getTankType() == Fluids.WATER){
        		test = new ResourceLocation(RefStrings.MODID, "textures/blocks/forgefluid/" + chem.tanks[0].getTankType().getUnlocalizedName().substring(11) + "_chemplant.png");
        	} else {
        	String s = chem.tanks[0].getTankType().getTexture().toString();
        ...
        if(chem.tanks[1].getTankType() != null) {
           	ResourceLocation test;
			if(chem.tanks[1].getTankType() == Fluids.LAVA || chem.tanks[1].getTankType() == Fluids.WATER){
        		test = new ResourceLocation(RefStrings.MODID, "textures/blocks/forgefluid/" + chem.tanks[1].getTankType().getUnlocalizedName().substring(11) + "_chemplant.png");
        	} else {
        	String s = chem.tanks[1].getTankType().getTexture().toString();
        	String textureBase = "textures/";
        	String[] test1 = s.split(":");
        	String location = test1[0] + ":" + textureBase + test1[1] + ".png";
        	test = new ResourceLocation(location);
