...
@AutoRegister
public class RenderAssembler extends TileEntitySpecialRenderer<TileEntityMachineAssembler> implements IItemRendererProvider {
    ...
    public void renderSlider(TileEntityMachineAssembler tileEntity, double x, double y, double z, float f) {
        ...
        if (tileEntity.isProgressing) GlStateManager.translate(offset * 0.003 - 0.75, 0, 0);

        ResourceManager.assembler_slider.renderAll();

        bindTexture(ResourceManager.assembler_arm_tex);

        double sway = (System.currentTimeMillis() % 2000) / 2D;

        sway = Math.sin(sway / Math.PI / 50);

        if (tileEntity.isProgressing) GlStateManager.translate(0, 0, sway * 0.3);
    ...
    public void renderCogs(TileEntityMachineAssembler assembler, double x, double y, double z, float f) {
        GlStateManager.pushMatrix();
        GlStateManager.translate(x, y, z);
        GlStateManager.enableLighting();
        GlStateManager.disableCull();
