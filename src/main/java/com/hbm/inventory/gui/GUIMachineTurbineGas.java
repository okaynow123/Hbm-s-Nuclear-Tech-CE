...
public class GUIMachineTurbineGas extends GuiInfoContainer {
	...
	protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
		...
		yStart = mouseY;
				...
				if(guiLeft + 80 <= mouseX && guiLeft + 80 + 16 > mouseX && guiTop + 45 < mouseY && guiTop + 45 + 15 >= mouseY) { //start-stop circular button
		...
		if(turbinegas.state == 1 && mouseX > guiLeft + 74 && mouseX <= guiLeft + 74 + 29 && mouseY >= guiTop + 86 && mouseY < guiTop + 86 + 13) { //auto mode button
	...
	protected void drawThermometer(int temp) {
		...
		bufferbuilder.pos(xPos + width, yPos + + 64 - (64 * temp / maxTemp), 0.0D).tex(uMax, vMin).endVertex();
		bufferbuilder.pos(xPos, yPos + + 64 - (64 * temp / maxTemp), 0.0D).tex(uMin, vMin).endVertex();
		tessellator.draw();
