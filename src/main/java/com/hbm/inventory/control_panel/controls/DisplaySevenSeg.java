...
//    D
    ...
    public void render() {
        ...
        int value = Math.max(0, (int) getVar("value").getNumber());
        ...
        for (int i = 0; i < digitCount; i++) {
            ...
            GlStateManager.translate(posX - t_off, 0F, posY);
            GlStateManager.color(1F, 1F, 1F, 1F);
            model.renderPart("base");
            GlStateManager.popMatrix();

            // border (untextured grey)
            GlStateManager.disableTexture2D();
            GlStateManager.pushMatrix();
            GlStateManager.translate(posX - t_off, 0F, posY);
            GlStateManager.color(0.31F, 0.31F, 0.31F, 1F);
            model.renderPart("border");
            GlStateManager.popMatrix();

            // segments (fullbright, untextured, per-segment color/intensity)
            OpenGlHelper.setLightmapTextureCoords(OpenGlHelper.lightmapTexUnit, 240, 240);
            for (int j = 0; j < 7; j++) {
                boolean enabled = (character & (1 << j)) != 0;
                float cMul = enabled ? 1F : 0.1F;
